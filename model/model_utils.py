...
class TuckerLayer(nn.Module):
    ...
    def regularizer(self, lambda_=1e-5):
        return lambda_ * torch.linalg.norm(self.linear.weight, ord='nuc')
...
def create_tuco_net_np_decoder_branch_ext(ksize=3, middle_channels=64):
    pad = ksize // 2
    module_list = [
...
def create_tuco_net_hv_decoder_branch_ext(ksize=3, middle_channels=64):
    pad = ksize // 2
    module_list = [
        nn.Conv2d(1024, 256, ksize, stride=1, padding=pad, bias=False),
        DenseBlockExt(256, [1, ksize], [128, 32], 8, split=4),
...
def create_tuco_net_tp_decoder_branch_ext(out_ch=2, ksize=3):
    pad = ksize // 2
    module_list = [
        nn.Conv2d(1024, 256, ksize, stride=1, padding=pad, bias=False),
        DenseBlockExt(256, [1, ksize], [128, 32], 8, split=4),
    ...
    module_list = [
        TuckerLayer(512, 502),
        nn.Conv2d(502, 128, ksize, stride=1, padding=pad, bias=False),
        DenseBlockExt(128, [1, ksize], [128, 32], 4, split=4),
        nn.Conv2d(256, 256, 1, stride=1, padding=0, bias=False)
    ]
    u2 = nn.Sequential(*module_list)
    ...
    module_list = [
        TuckerLayer(256, 246),
        nn.Conv2d(246, 64, ksize, stride=1, padding=pad, bias=False)
    ]
    u1 = nn.Sequential(*module_list)
...
class TuDeMod(nn.Module):

    def __init__(self, in_channels, reduced_channels):
        super(TuDeMod, self).__init__()
...
def create_tude_net_np_decoder_branch_ext(ksize=3, middle_channels=64):
    pad = ksize // 2
    module_list = [
        nn.Conv2d(1024, 256, ksize, stride=1, padding=pad, bias=False),
        DenseBlockExt(256, [1, ksize], [128, 32], 8, split=4),
...
def create_tude_net_hv_decoder_branch_ext(ksize=3, middle_channels=64):
    pad = ksize // 2
    module_list = [
        nn.Conv2d(1024, 256, ksize, stride=1, padding=pad, bias=False),
        DenseBlockExt(256, [1, ksize], [128, 32], 8, split=4),
...
def create_tude_net_tp_decoder_branch_ext(out_ch=2, ksize=3):
    pad = ksize // 2
    module_list = [
        nn.Conv2d(1024, 256, ksize, stride=1, padding=pad, bias=False),
        DenseBlockExt(256, [1, ksize], [128, 32], 8, split=4),
...
def create_tude_net_tp_decoder_branch_ext_old(out_ch=2, ksize=3):
    pad = ksize // 2
    module_list = [
        nn.Conv2d(1024, 256, ksize, stride=1, padding=pad, bias=False),
        DenseBlockExt(256, [1, ksize], [128, 32], 8, split=4),
