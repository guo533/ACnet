...
import base.base_model_utils as base_model_utils
from base.base_model_utils import UpSample2x
...
class HoVerNetExt(nn.Module):
    ...
    def __init__(self, input_ch=3, num_types=None, freeze=False, pretrained_encoder_path=None):
        ...
        self.freeze = freeze
        self.backbone = base_model_utils.resnet50(self.input_ch, pretrained_path=pretrained_encoder_path)
        self.conv_bot = nn.Conv2d(2048, 1024, 1, stride=1, padding=0, bias=False)
        if num_types is None:
            decoder = OrderedDict(
                [
                    ("np", base_model_utils.create_decoder_branch_ext(out_ch=2, ksize=3)),
                    ("hv", base_model_utils.create_decoder_branch_ext(out_ch=2, ksize=3))
                ]
            )
        else:
            decoder = OrderedDict(
                [
                    ("np", base_model_utils.create_decoder_branch_ext(out_ch=2, ksize=3)),
                    ("tp", base_model_utils.create_decoder_branch_ext(out_ch=num_types, ksize=3)),
                    ("hv", base_model_utils.create_decoder_branch_ext(out_ch=2, ksize=3))
                ]
            )
