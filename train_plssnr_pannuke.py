...
def create_perturbation_counterparts(unlabeled_images, raw_pred_dict):
    ...
    # unlabeled_pred_decoder_outputs = unlabeled_pred_dict['decoder_output']
    # _, unlabeled_pred_type_maps = process_utils.post_process(unlabeled_pred_type)
    # unlabeled_pred_type_maps = utils.ensure_array(unlabeled_pred_type)
    ...
    # pert_unlabeled_pseudo_type_maps = torch.from_numpy(np.concatenate(pert_unlabeled_pseudo_type_map_list, axis=0)).to(
    #     utils.device()).type(torch.float32)
    #
    pert_unlabeled_pseudo_type_maps = F.one_hot(pert_unlabeled_pseudo_type_maps, num_classes=num_types)
    # unlabeled_pred_type_maps = torch.from_numpy(utils.one_hot(unlabeled_pred_type_maps, num_classes=num_types)).to(utils.device()).type(torch.int64)
...
def train(opt):
    # 0, 1, 2, 3, 4, 5
    memory_bank = [0, 0, 0, 0, 0]
    memory_bank_count = [0, 0, 0, 0, 0]
    ...
    # simsiam_net_optimizer.load_state_dict(torch.load(opt.pretrained_checkpoint_path, map_location=utils.device())['simsiam_optimizer'])
    ...
    labeled_dataloader = DataLoader(FileLoaderExt(labeled_file_list, mode='train', with_type=True, input_shape=(256, 256),
                                        mask_shape=(256, 256), use_affine=False), batch_size=opt.batch_size, shuffle=True, num_workers=opt.workers, pin_memory=True)
    unlabeled_dataloader = DataLoader(FileLoaderExt(unlabeled_file_list, mode='val', with_type=True, input_shape=(256, 256),
                                        mask_shape=(256, 256), use_affine=False), batch_size=2, shuffle=False, num_workers=opt.workers, pin_memory=True, drop_last=True)
    ...
    for epoch in range(opt.warmup_epoch + 1, opt.epochs):
        ...
        for batch_idx, batch_data in enumerate(labeled_dataloader):
            ...
            with amp.autocast():
                ...
                true_inst_type_maps = np.zeros((batch_size, labeled_true_instance_map_array.shape[1], labeled_true_instance_map_array.shape[2], opt.num_types - 1), dtype=np.int64)
                for b in range(batch_size):
                    true_inst_type_map = utils.create_ann_inst_type_map(labeled_true_instance_map_array[b], labeled_true_tp_array[b], opt.num_types)
                    ...
                    for b in range(batch_size):
                        ...
                        if len(inst_list) > 1:
                            ...
                            for inst_id in inst_list:
                                ...
                                instance_feat = labeled_tp_feat[b, ...] * torch.from_numpy(tmp_matrix).to(utils.device())
                                instance_feat = torch.masked_select(instance_feat, torch.from_numpy(tmp_matrix).to(utils.device()))
                                # instance_feat = F.interpolate(torch.unsqueeze(torch.unsqueeze(instance_feat, 0), 0), 5000)
                                instance_feat = F.interpolate(torch.unsqueeze(torch.unsqueeze(instance_feat, 0), 0), feature_size)
                ...
                labeled_pred_postprocessed_output_prototypes = [None, None, None, None, None]
                ...
                labeled_intra_loss = calc_loss_if_not_none(hb_losses.mse_loss, memory_bank[0] / (memory_bank_count[0] + 1e-8), labeled_pred_postprocessed_output_prototypes[0]) \
                    + calc_loss_if_not_none(hb_losses.mse_loss, memory_bank[1] / (memory_bank_count[1] + 1e-8), labeled_pred_postprocessed_output_prototypes[1]) \
                    + calc_loss_if_not_none(hb_losses.mse_loss, memory_bank[2] / (memory_bank_count[2] + 1e-8), labeled_pred_postprocessed_output_prototypes[2]) \
                    + calc_loss_if_not_none(hb_losses.mse_loss, memory_bank[3] / (memory_bank_count[3] + 1e-8), labeled_pred_postprocessed_output_prototypes[3]) \
                    + calc_loss_if_not_none(hb_losses.mse_loss, memory_bank[4] / (memory_bank_count[4] + 1e-8), labeled_pred_postprocessed_output_prototypes[4])
            ...
            if epoch <= opt.warmup_epoch:
                ...
                with amp.autocast():
                    ...
                    pert_unlabeled_pred_postprocessed_output_prototypes = [None, None, None, None, None]
                        ...
                        for b in range(pert_batch_size):
                            ...
                            if len(inst_list) > 1:
                                ...
                                for inst_id in inst_list:
                                    ...
                                    tmp_matrix = tmp_matrix[..., None]
                                    instance_feat = pert_unlabeled_tp_feat[b, ...] * torch.from_numpy(tmp_matrix).to(
                                        utils.device())
...
def parse_opt():
    ...
    parser.add_argument('--labeled-dataset-dir', type=str, default=r'D:\CJH\data\generated\半监督PanNuke\labeled')
    parser.add_argument('--unlabeled-dataset-dir', type=str, default=r'D:\CJH\data\generated\半监督PanNuke\unlabeled')
    ...
    parser.add_argument('--labeled-intra-lambda', type=float, default=0.01)
    parser.add_argument('--pert-unlabeled-intra-lambda', type=float, default=0.01)
