...
def produce_cam(opt):
    ...
    opt.dataset_dir = os.path.join(opt.dataset_dir, f'{opt.file_index}')
    ...
    for batch_idx, batch_data in enumerate(val_dataloader):
        ...
        for i in range(num_patches):
            ...
            with GradCAM(model=model, target_layers=target_layers) as cam:
                grayscale_cam = cam(input_tensor=torch.unsqueeze(imgs[i], 0), targets=targets)[0, ...]
                cam_image = show_cam_on_image(batch_data['img'][i].numpy() / 255.0, grayscale_cam, use_rgb=True)[:1000,:1000]
                cam_images.append(cam_image)
                file_idx += 1
        full_map = np.zeros((1024, 1024, 3))
        image_index = 0
        for col in range(0, 1024, 256):
            for row in range(0, 1024, 256):
                full_map[0, col: col + 256, row: row + 256, ...] = cam_images[image_index]
                image_index += 1
        final_full_map = full_map[:1000, :1000]
        plt.figure(dpi=100)
        plt.imshow(final_full_map)
        plt.axis('off')
...
def parse_opt():
    ...
    parser.add_argument('--file-index', type=int, default=9)
