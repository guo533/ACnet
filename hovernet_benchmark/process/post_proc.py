...
def proc_np_hv(pred):
    ...
    blb = utils.remove_small_objects(blb, min_size=10)
    ...
    marker = utils.remove_small_objects(marker, min_size=10)
...
def process_prob_map_with_hv_map(pred_map):
    """Process mask with horizontal and vertical maps
    Args:
        pred_map: commbined output of np and hv branches, in the same order
...
def process_prob_map_without_hv_map(pred):
    """Process mask without horizontal and vertical maps"""
...
def produce_inst_type_maps_for_test_cic_nsc(pred_map, num_types=None, return_centroids=False):
    """Post processing script for image tiles.
    Args:
        pred_map: commbined output of tp, np and hv branches, in the same order
        num_types: number of types considered at output of nc branch
