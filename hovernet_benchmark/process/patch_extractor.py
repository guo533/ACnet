...
class PatchExtractor(object):
    ...
    def extract(self, x, patch_type):
        ...
        if patch_type == "valid":
            return self._extract_valid(x)
        elif patch_type == "mirror":
            return self._extract_mirror(x)
        else:
            assert False, "Unknown Patch Type [%s]" % patch_type

    def _get_patch(self, x, ptx):
        ...
        if self.debug:
            if self.patch_type == "mirror":
                cen = utils.cropping_center(win, self.step_size)
    ...
    def _extract_valid(self, x):
        ...
        if h_flag and w_flag:
            ...
            win = self._get_patch(x, ptx)
            sub_patches.append(win)
        return sub_patches

    def _extract_mirror(self, x):
        ...
        sub_patches = self._extract_valid(x)
        return sub_patches
